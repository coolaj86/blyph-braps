...
*/
  ...
  // TODO allow non-uuid
  function restfullyUpdateUser(req, res) {
    if (!req.session.uuid) {
      res.error(new Error('not logged in'));
    }
    if (!req.params.id) {
      res.error(new Error('no user uuid given'));
    }
    if (req.session.uuid !== req.params.id) {
      res.error(new Error("attempting to update a user that isn't you (and you aren't an admin)"));
    }
    if (res._errors && res._errors.length > 0) {
      res.json();
      return;
    }
    ...
    function respondWithAccount(err, account) {
      ...
      req.session.school = account.school;
      setUserAccount(account);
      res.json(req.session);
    }

    // TODO make generic, but don't overwrite password, otp, etc
    directRetrieveUser(respondWithAccount, req.params.id);
  }

  function init(pathname, addAccountToSession) {
    var demoDb = {
            "coolaj86": {
                "passphrase": "secret"
              , "username": "coolaj86"
  ...
  module.exports.restfullyUpdateUser = restfullyUpdateUser;
}());
